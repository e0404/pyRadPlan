...
- A documented and engine-tested custom biological-model extension contract. `register_model` is usable as a decorator, validates model declarations, and checks every name before modifying the registry; dose engines validate custom evaluator bindings, declarations, per-energy kernel fields and returned quantity names at their public boundaries.
- Biological models declare the reference photon parameters their formulation needs to be positive (`BiologicalModel.requires_positive_reference`); the dose engines validate that declaration against the `alpha_x` / `beta_x` of all structures before a calculation starts. The RBEmin/RBEmax models need `alpha_x > 0`, McNamara additionally `beta_x > 0`
- `DoseEngineBase.engine_provided_quantities()`: quantities an engine produces without machine data. Biological availability is checked against the machine's quantities plus these, so FRED accepts LET-based models for machines without pencil-beam LET kernels
- `xp_utils.device_cache_key()`: hashable identity of an array's device for use as a dictionary key. It prefers the backend's own device object (which distinguishes `array_api_strict`'s logical devices, all of which share the DLPack tuple `(1, 0)`) and falls back to the DLPack `(device_type, device_id)` tuple only for backends whose device objects are unhashable
...
- `Dij.bio_model`: the biological model the dose engine used (validated like `Plan.bio_model`; `None`, dict or instance), serialised as its parameter dict; `dij.rbe` is derived from it for constant-RBE models and a legacy scalar `rbe`/`RBE` (matRad) is folded into a `constant_rbe` model on import and written back as `RBE` on export. `alphax`/`betax` carry one column per CT scenario
- `rbe_x_dose` quantity (`RBExDose`) with two paths decided by `dij.bio_model` (`RBExDose.choose_path`): LQ inversion of the `effect` (`lq_inverse_dose`) or the constant RBE; models providing alpha/beta without matrices on the dij raise a clear error. Result assembly in `Dij` uses the same rule and inversion
- `QuantityResolver.try_get`; optional quantity dependencies that cannot be computed for a dij are skipped instead of failing the resolution
...
- docs: biological models, `dose_convention` and the `"auto"` quantity switches in the user guide; `bio_models` API page
- matRad interoperability of biological models: `pln.bioModel` names (`constRBE`, `LEM`) and model structs are understood on import (`bio_model_spec_from_matrad`); `Plan.to_matrad()` writes the matRad model name and preserves explicitly configured parameters in `bioModelParameters` for a lossless pyRadPlan `.mat` round trip
- `bio_influence_from_let` derives the evaluator's declared biological influence matrices from physical dose and LET·dose matrices; the FRED engine uses it for LET-based models (`calc_bio_dose="auto"`) and stores the LET matrix under the canonical `let_dose` name
...
- Every dose engine now records the biological model it used on the dij, so a constant RBE works through the common result assembly for photons as well; an engine that cannot compute a model's declared outputs logs a warning instead of silently dropping the model
- `RBExDose` with a constant RBE scales the physical-dose vector instead of building a scaled copy of the influence matrix on every evaluation, and chains its derivative through the physical-dose quantity
- `ExactClassLookup` converts the reference tissue classes into the caller's namespace/device/dtype once instead of per bixel, and skips the redundant per-bixel match check (and its device synchronization) once the whole dose grid has been validated. It also rejects empty or inconsistent class declarations
...
- LET-based biological models produced NaN/Inf `alpha_dose` / `sqrt_beta_dose` entries for invalid or degenerate reference photon parameters (McNamara with `beta_x = 0`, every RBEmin/RBEmax model for `alpha_x = beta_x = 0`, and any negative, infinite or NaN `alpha_x` / `beta_x` — including Wedenberg's `beta_x`, which no model declares as positive), and `bio_influence_from_let` stored them in the sparse influence matrices. Dose engine setup now rejects reference coefficients that are not finite and non-negative for every model, then applies each model's stricter positivity declaration; voxels outside every structure evaluate to `alpha = beta = 0`; and the influence helper raises if a model still returns non-finite values
- Fragment spectra aggregating all isotopes of a charge (`A = NaN`) were dropped from the tabulated RBE models: ordinary equality never matches such an entry, so a spectrum of hydrogen plus aggregated carbon silently kept hydrogen only. Species are now matched by the documented aggregation convention (`match_fragment_species`), which `ChargedBeamFragmentSpectrum.get` uses as well
- `kernel_based_lq` derived the tissue class order from the first machine energy alone, so reordered tissue rows at another energy silently selected the wrong alpha/beta kernel row. The tissue classes of every energy are now validated to be identical and in the same order, and missing or inconsistent class metadata is reported clearly
- The particle pencil-beam engine created the per-voxel `alpha_x` / `beta_x` arrays without the engine's device, so a non-default device failed when they were combined with the physical-dose and LET arrays
- Photon dose calculation dropped an accepted constant-RBE model: the dij carried neither `bio_model` nor `rbe`, so `rbe_x_dose` was unavailable
- `ParticlePencilBeamKernel` could not be rebuilt from its own `model_dump()` and rejected an existing `ChargedBeamFragmentSpectrum`: the matRad parser ran unconditionally (`KeyError: 'spectra'`), and absent optional kernel arrays were turned into 0-d NaN arrays
- FRED rejected LET-based models for machines without pencil-beam LET kernels although it scores LET itself, and accepted evaluators requiring further machine kernel inputs it cannot supply; both are now decided by engine-aware capability checks before a simulation starts. The input check is applied to the evaluator itself rather than to the model's intrinsic `output_quantities`, so an evaluator producing the additive influence quantities directly cannot bypass it
- The helium tutorial plotted outside the GUI fallback, so it requested `rbe_x_dose` from a plan without a biological model and used an undefined `view_slice` when the GUI is available. It now selects the Mairani helium model and plots inside the fallback branch
- The biological model table in the plan user guide listed modalities the models do not declare (oxygen for `LSM`, and protons/helium missing for `dose_average_alpha_beta`)
- `TabulatedSpectrumEvaluator` cached its per-energy kernel tables under a key containing the backend device object, which raised `TypeError: unhashable type: 'cupy.cuda.device.Device'` on the CuPy backend. Both this cache and the new tissue-lookup cache are keyed by the DLPack `(device_type, device_id)` tuple
- The oxygen tutorial plotted `rbe_x_dose` although oxygen defaults to no biological model; it now selects `kernel_based_lq` explicitly, matching the Generic oxygen base data
...
- `OptInfo` TypedDict for retrieving objective history, iteration count, and solver result info from `fluence_optimization` via the keyword-only `opt_info` argument
- Objective function history tracking in `NonLinearFluencePlanningProblem`
- Solver configuration from dictionary in `get_solver`: `{"name": "ipopt", "options": {...}}` with attribute merging for dict-typed options
- `display` and `max_iter` properties on `PlanningProblem`, propagated to the solver by `PlanningProblem._initialize` for every problem subclass. Both are unset by default: a solver configured through `prop_opt["solver"]` keeps its own `max_iter`/`display`, and only a value assigned at the top level takes precedence over it. The forwarded knobs are listed in the `solver_properties` class variable; one the solver does not define is logged as ignored instead of being attached to it, so a non-iterative solver (direct inversion, an end-to-end model) is not given a meaningless `max_iter`
- `OptimizerIpopt.output_mode` (`"auto"`, `"native"`, `"logging"`) controlling where IPOPT's output goes. IPOPT writes its iteration table from C straight to the process' standard output descriptor, which Jupyter does not display; `"logging"` captures that descriptor and re-emits every line through Python's `logging` (including the convergence summary and the `EXIT:` line, which the iteration callback does not expose), `"native"` leaves it in place, and the default `"auto"` picks native in a terminal and logging in a notebook. `display=False` still silences IPOPT either way
- `native_output_to_logger` context manager in `pyRadPlan.util` routing output written to an OS file descriptor through a logger, for extension libraries (and child processes) that bypass `sys.stdout`. It declines to capture when a handler of the receiving logger writes to that same descriptor, which would feed the logger's own records back into the pipe and never terminate (`logging.lastResort` counts, so capturing descriptor 2 with no logging configured is refused too). A second capture of a descriptor already being captured is refused as well, since restoring on exit would otherwise leave the descriptor pointing at a dead pipe
- Iteration logging with objective value for `OptimizerSciPy`
- `warnings_to_logger` context manager routing `warnings.warn` through `logging`, including the warning category. Ignore filters set by the user or the stdlib keep applying; only warnings that would otherwise be dropped by the once-per-location default are caught. Because `warnings.catch_warnings` is not thread-safe before Python 3.14, a hook that another thread's `catch_warnings` restores after the block has ended passes warnings on to the function it replaced instead of swallowing them
- `detect_jupyter()` helper in `pyRadPlan.util._jupyter`, used to resolve `OptimizerIpopt.output_mode="auto"`
- Example `examples/utils_optimization.py` for solver and optimization settings
...
- Lint: the ruff/pylint complexity thresholds are relaxed for scientific code (`max-args` 5 -> 10, `max-returns` 6 -> 12, `max-branches` 12 -> 20, `max-statements` 50 -> 80), so the rules flag genuine outliers instead of ordinary numerical code, where physics kernels take many parameters and backend dispatchers branch on dtype/backend/modality. The `PLR0913` per-file ignores for `raytracer/_kernels*.py` are no longer needed and were removed
- GUI: Save Workspace now offers only pickle so that all pipeline objects, fluence weights, result quantities, and saved snapshot tags survive reloading. MATLAB and NPZ are no longer offered for complete-workspace saves because those paths dropped workspace data; dedicated object and image exports remain available. Saving a workspace without a CT is rejected before opening the file dialog
...
- Solver warnings and fallback messages use `logging` instead of `warnings.warn`
- `get_solver` raises `ValueError` for unknown solver names instead of silently continuing
- Solver `display` flag controls IPOPT `print_level` and SciPy iteration output
- **Breaking:** solvers report their result as a dictionary that always carries a normalized `num_iter`, so the iteration count no longer depends on solver-specific key names. `OptimizerIpopt` returned ipyopt's bare integer status code, violating the `tuple[Array, dict]` contract of `SolverBase.solve` and leaving `opt_info["num_iter"]` unset and `opt_info["result_info"]` a plain `int` for the default solver; it now returns a dictionary with `status`, `objective` and `num_iter`. Code that unpacked the second return value of `OptimizerIpopt.solve()` as an integer status must read `result_info["status"]` instead
- A solver requested explicitly via `pln.prop_opt["solver"]` that is not registered now raises `ValueError` instead of silently falling back to another solver, which would discard its configuration and change the optimization result. Only the default (`"ipopt"`) still falls back, with a warning naming the missing backend and the fallback's option names
...
- Photon SVD pencil-beam dose calculation crashed on the PyTorch backend (`test_photons_field_based`): the per-beamlet field masks were stored as the negatively strided view returned by `np.rot90`, and PyTorch accepts neither `torch.from_numpy` nor a DLPack import of such an array -- on Windows the DLPack path aborts the process rather than raising. The masks are now materialized once when the beam is set up, and `xp_utils.to_namespace`/`from_numpy` copy any negatively strided NumPy array before handing it to PyTorch
- Converting a PyTorch sparse tensor to a CuPy sparse matrix (`to_namespace(cp, ...)`) failed for every layout and source device: the GPU path passed a `copy=` keyword that `cupy.from_dlpack` does not accept, and the CPU path handed the cupyx constructors NumPy arrays, which they reject ("data, indices, and indptr should be 1-D"). The GPU path now hands the tensors over via DLPack unchanged and the CPU path uploads them with `cp.asarray`; covered by `test_torch_sparse_to_cupy_preserves_format` for COO/CSR/CSC from both a CPU and a CUDA source
- GUI: loading a new patient CT previously retained any pipeline objects absent from the import, leaving old structures, plans, or dose paired with the new patient. Imports containing a CT now replace the patient in one workspace update and explicitly restore or clear snapshot tags; imports without a CT still update the current patient
- Image import: label maps on a different grid previously had their physical geometry overwritten with the CT metadata. They are now resampled with nearest-neighbor interpolation and zero background, with warnings when resampling is needed and when a label becomes empty on the CT grid
- GUI: applying plan edits previously rebuilt the plan from only the visible fields, discarding solver settings and other unedited properties. Apply now preserves optimization settings, prescription, and unedited steering and sequencing parameters; steering and sequencing settings reset when the radiation modality changes
- GUI: "Recalculate Dose" always failed with an `AttributeError` and never produced a dose. It called `compute_result_ct_grid(weights)` on the return value of `calc_dose_forward`, which is a result dictionary on the CT grid rather than a `Dij`; had the call succeeded it would have applied the weights a second time, since the engine already folds them into the beamlets before calculating. The result dictionary is now used directly, and `calc_dose_forward` annotates and documents `dict` as its return type instead of `Dij`
- matRad: structure-set validation now normalizes NumPy object cell arrays before detecting the input format, restoring support for the array representation alongside equivalent Python lists
- AI: model discovery previously prepended the configured default whenever any provider had an API key, even if that default belonged to a provider without credentials. Defaults from recognized providers are now suggested only when their own provider's key is present
...
- `OptimizerSciPy.display` was declared but never assigned, so `prop_opt["solver"]` / `get_solver({"name": "scipy", "display": ...})` warned "property not found" and dropped it; it is now a real attribute that logs the objective value per iteration. SciPy's own `disp` option is not used, as it is deprecated (removal in SciPy 1.18) and already silent for L-BFGS-B
...
- `OptimizerIpopt` assembled each solve's options by mutating its stored `options` dictionary: `display=False` set `print_level` to 0 permanently, so re-enabling `display` left IPOPT silent and a user-chosen `print_level` was lost after one silenced solve; and `max_iter`/`max_cpu_time`/`acceptable_tol` given inside `options` were silently overwritten by the solver attributes. Options are now assembled per solve on a copy; the three attribute-backed keys are no longer seeded into `options`, and a value placed there for one of them is logged as ignored
- `KeyboardListener.finalize` incorrectly reporting "aborted by user" when convergence set the stop event
- IPOPT `max_iter` not being forwarded from `PlanningProblem`
- `OptimizerSciPy.abs_obj_tol` had no effect: SciPy fills the tolerance option its method understands from `minimize(tol=...)` with `setdefault`, so the `ftol`/`gtol` this solver seeded into `options` silently discarded it and every tolerance produced an identical run. Those two are no longer seeded, and `abs_obj_tol` defaults to `1e-5` for this solver — the tolerance it effectively ran with before — so making the knob work does not change existing results. A tolerance set explicitly in `options` still takes precedence for that tolerance, as it does in SciPy itself (a single objective tolerance cannot express independently tuned `ftol`/`gtol`/`xtol`); configuring both is reported so the one that loses is not silent
- `OptimizerSciPy.max_iter` had no effect with `method="TNC"`: SciPy's TNC takes its iteration cap as `maxfun` and reports `maxiter` as an unknown option, so the run was never capped. The option name is now chosen per method
- `OptimizerSciPy` assembled each solve's options by mutating its stored `options` dictionary, so a `maxiter` set through `prop_opt["solver"]` was overwritten by `max_iter` and lost from the dictionary. As for IPOPT, options are now assembled per solve on a copy and an iteration cap placed in `options` is logged as ignored
- `fluence_optimization` never cleared the optional keys of a caller-supplied `opt_info`, so reusing one across solves reported `obj_history` / `num_iter` from an earlier run when the current one did not fill them (including when it raised). The keys are dropped before the solve
- `opt_info["obj_history"]` was reported as an empty list — indistinguishable from a solve that never evaluated the objective — for a planning problem that does not record one. Problems declare this via the `records_obj_history` class variable, and the key is left unset (as `num_iter` already was) with a warning naming the problem
- `PlanningProblem.solve` appended to an existing `obj_history` instead of starting a new one, so a second solve on the same problem returned both runs concatenated. The list is now cleared in place at the start of each solve
- `native_output_to_logger` left the captured descriptor pointing at a pipe with no reader, and leaked that pipe plus the saved descriptor, when setting up the capture failed part-way (e.g. the draining thread could not start). The next write to the descriptor would then block once the pipe filled. The descriptor is restored and everything opened is closed before the failure propagates
