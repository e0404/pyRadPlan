...
from pyRadPlan.quantities import ConstantRbeXDose, FluenceDependentQuantity
...
def test_ConstantRBE_constructor(sample_dij_dense):
    const_rbe = ConstantRbeXDose(sample_dij_dense)
    assert isinstance(const_rbe, FluenceDependentQuantity)
    assert const_rbe.mode == "indirect"
    assert const_rbe.scenarios == [0]
    assert const_rbe._dij == sample_dij_dense.to_namespace(xp)
    assert const_rbe.dim == 1
    assert format(const_rbe.unit, "~") == "Gy"
    assert const_rbe.identifier == "constant_rbe_x_dose"
    assert const_rbe.name == "constant_rbe_x_dose"


def test_ConstantRBE_dense(sample_dij_dense):
    const_rbe = ConstantRbeXDose(sample_dij_dense)
    rbe = 1.1
...
def test_ConstantRBE_sparse(sample_dij_sparse):
    const_rbe = ConstantRbeXDose(sample_dij_sparse)
    rbe = 1.1
