...
class RBExDose(FluenceDependentQuantity):
    ...
    def _compute_quantity_single_scenario(self, scenario_index: int) -> Array:
        ...
        ix = self._dij.betax[:, scenario_index] > 0
        gamma = self.array_backend.zeros_like(self._dij.betax[:, scenario_index])
        gamma[ix] = (
            self._dij.alphax[:, scenario_index][ix] / self._dij.betax[:, scenario_index][ix] / 2
        )
        rbe_x_dose = self.array_backend.zeros_like(effect.flat[scenario_index])
        rbe_x_dose[ix] = (
            self.array_backend.sqrt(
                gamma[ix] ** 2
                + effect.flat[scenario_index][ix] / self._dij.betax[:, scenario_index][ix]
            )
            - gamma[ix]
        )
        return rbe_x_dose
    ...
    def _compute_chain_derivative_single_scenario(self, d_quantity, scenario_index: int) -> Array:
        ...
        ix = self._dij.betax[:, scenario_index] > 0
        gamma = xp.zeros_like(self._dij.betax[:, scenario_index])
        gamma[ix] = (
            self._dij.alphax[:, scenario_index][ix] / self._dij.betax[:, scenario_index][ix] / 2
        )
        ...
        fgrad[ix] = d_quantity[ix] / (
            2 * self._dij.betax[:, scenario_index][ix] * effect.flat[scenario_index][ix]
        )
